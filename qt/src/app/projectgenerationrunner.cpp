...
QVector<ProjectIpInstanceRecord> generationInstancesFromProjectDesign(
    const ipcraft::core::ProjectDesign& design,
...
ProjectGenerationResult ProjectGenerationRunner::generate(const ProjectGenerationRequest& request) const {
    ...
    const QVector<ProjectIpInstanceRecord> generationInstances =
        generationInstancesFromProjectDesign(*request.projectDesign, request.catalogEntries);
    ...
    for (const ProjectIpInstanceRecord& instance : generationInstances) {
        if (!isSafeInstanceOutputKey(instance.instanceId)) {
