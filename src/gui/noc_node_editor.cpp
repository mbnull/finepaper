...
constexpr qreal kRouterHorizontalSpacing = 250.0;
constexpr qreal kRouterVerticalSpacing = 190.0;
constexpr qreal kEndpointHorizontalOffset = 125.0;
...
public:
    ...
    void configure(QString caption, bool router) {
        ...
        m_inputPorts = 1;
        m_outputPorts = router ? 1 : 1;
        setBackgroundColor(router ? QColor(QStringLiteral("#dceef7"))
                                  : QColor(QStringLiteral("#f7e2b8")));
...
public:
    ...
    QtNodes::NodeFlags nodeFlags(QtNodes::NodeId nodeId) const override {
        ...
        if (m_lockedNodes.contains(nodeId)) {
    ...
    bool deleteNode(QtNodes::NodeId const nodeId) override {
        ...
        m_lockedNodes.remove(nodeId);
        return QtNodes::DataFlowGraphModel::deleteNode(nodeId);
    }

    QtNodes::NodeId addProjectedNode(QString caption, QPointF position, bool locked) {
        ...
        if (auto* model = delegateModel<NocNodeModel>(nodeId)) {
            model->configure(std::move(caption), locked);
        }
        if (locked) {
            m_lockedNodes.insert(nodeId);
...
private:
    ...
    QSet<QtNodes::NodeId> m_lockedNodes;
};
...
void NocNodeEditor::rebuildGraph() {
...
void NocNodeEditor::handlePointerReleased() {
    if (!endpointMoveRequested || !m_scene) {
        return;
    }
