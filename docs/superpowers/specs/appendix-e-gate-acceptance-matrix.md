...
`ipcraft.fixture-catalog.v1` catalogs only deterministic validation of one standalone JSON document by JSON Schema or by a Core-semantic validator. Its closed entries contain exactly `path`, `schemaId`, `validationPhase`, `expected`, `errorCode`, and `behaviorEvidence`; entries are sorted by portable normalized `path` and cover every physical file below `fixtures/valid/` and `fixtures/invalid/` exactly once. `schemaId` and non-null `errorCode` resolve the schema and error catalogs. Reject classification must also match the closed `(schemaId, validationPhase) -> allowed stable error codes` mapping in `fixture-error-policy-v1.json`; global error-catalog membership alone is insufficient. Accept entries use the valid prefix, null `errorCode`, and may point to one exact committed vector case as `vectors/<file>.json#<case-id>`; the file, envelope/kind, and case-bearing array must resolve through the closed vector catalog. Reject entries use the invalid prefix, a non-null stable error, and null `behaviorEvidence`.

Schema-catalog paths are NFC portable relative paths matching `schemas/*.schema.json` and must resolve to regular, non-symlink files contained by `docs/contracts/schemas`; absolute paths, dot segments, separator/control hazards, and filesystem escapes are invalid. Portable path NFC and collision folding use only the committed Unicode 17 data described by Appendix C. Gate verification runs the full committed official normalization conformance source, checks the full-table integrity digests/counts, and—when official authoring sources are available—regenerates and byte-compares the committed normalization and case-fold artifacts.
