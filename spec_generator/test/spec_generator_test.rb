...
class SpecGeneratorTest < Minitest::Test
  ...
  def test_generates_ravenoc_extension_runtime_bundle
    Dir.mktmpdir do |dir|
      ...
      assert_includes modules_xml, '<module name="RaveNoC" palette_label="RaveNoC" graph_group="noc_core"'
      assert_includes modules_xml, '<identity external_id_prefix="ravenoc" display_prefix="RNoC" width="2" supports_mesh_coordinates="false" />'
      ...
      assert_includes modules_xml, '<port id="irq" direction="output" type="interrupt" bus_type="interrupt_array" role="status" name="IRQ" description="RaveNoC interrupt array" interface="irq" />'
      ...
      assert_includes graphics_xml, '<anchor ref="clock_reset" x="84" y="0" normal_x="0" normal_y="-1" label="CLK/RST" label_x="84" label_y="18" />'
  ...
  def test_rejects_extension_view_refs_missing_from_spec
    Dir.mktmpdir do |dir|
      extension_path = write_file(dir, 'spec/noc/ravenoc.yml', ravenoc_extension_yaml)
      write_file(dir, 'spec/noc/views/RaveNoC.xml', ravenoc_view_xml.sub('ref="irq"', 'ref="debug"'))
      ...
      assert_match(/view RaveNoC references unknown interface debug/, error.message)
    end
  end
